...
SELECT JOB_ID, first_name, last_name, hire_date, salary 
FROM employees;
...
FROM employees
WHERE  commission_pct > 0; --IS NOT NULL로 처리도 가능
...
SELECT first_name|| ' '|| last_name|| '''사원의 급여는 '|| salary||'달러 입니다.' AS Num13
FROM employees
...
WHERE job_id 
LIKE'%MAN%' 
ORDER BY job_id; --asc desc작성 안해도 작동함(default는 오름차순)
