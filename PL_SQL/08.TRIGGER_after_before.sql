...
/*
AFTER트리거  - INSERT, UPDATE, DELETE 작업이후에 동작하는 트리거
BEFORE트리거 - INSERT, UPDATE, DELETE 작업이전에 동작하는 트리거

:OLD = 참조 전 열의 값(INSERT: 입력전 자료, UPDATE:수정 전 자료, DELETE: 삭제될 값)
:NEW = 참조 전 열의 값(INSERT: 입력할 자료, UPDATE:수정 된 자료) --NEW를 실행할때는 이미 삭제될값을 삭제되어 없어졌으므로 DELETE는 없다!

테이블에 UPDATE나 DELETE를 시도하면 수정, 또는 삭제된 데이터를 
별도의 테이블에 보관해 놓는 형식으로 트리거를 사용
*/
...
--AFTER TRIGGER
CREATE OR REPLACE TRIGGER trig_user_backup 
...
DECLARE
    v_type VARCHAR2(10);
BEGIN
    IF UPDATING THEN --UPDATING = 시스템 자체에서 상태에 대한 내용을 지원하는 빌트인 구문
        v_type := '수정';
    ELSIF DELETING THEN --DELETING = 시스템 자체에서 상태에 대한 내용을 지원하는 빌트인 구문
        v_type := '삭제';
    END IF;
    
    --실행구문 작성
    INSERT INTO tbl_user_back_up
...
INSERT INTO tbl_user VALUES('test04', '희망이','대전');

/*
트리거의 활용
INSERT ->주문 테이블 -> 주문테이블 INSERT 트리거 실행(물품 테이블 UPDATE)
*/
...
DECLARE
    ...
    v_product_no NUMBER;
BEGIN
    dbms_output.put_line('trg_order_history activate!');
    SELECT
        :NEW.total
    INTO
        v_total
