...
--CASCADE 없을시 ->테이블이나 시퀀스를 생성해놓은 계정이라면 삭제가 진행되지않음
...
--USER_TABLESPACE테이블 스페이스를 기본 사용 공간으로 지정.
ALTER USER test1 DEFAULT TABLESPACE user_tablespace
QUOTA UNLIMITED ON user_tablespace;

--tablespace 내 전체 객체를 삭제
DROP TABLESPACE user_tablespace INCLUDING CONTENTS; --단 DBF파일은 남는다!
